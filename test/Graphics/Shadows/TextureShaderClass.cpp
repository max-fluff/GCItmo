...
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"./Shaders/textureShader.hlsl", L"./Shaders/textureShader.hlsl");
	if(!result)
	{
		return false;
	}
...
bool TextureShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, Matrix worldMatrix, Matrix viewMatrix, 
								Matrix projectionMatrix, ID3D11ShaderResourceView* texture,int index)
{
	bool result;
	this->index=index;
											 ...
											 Matrix projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	dataPtr->index = DirectX::SimpleMath::Vector4(index,index,index,1.0f);
